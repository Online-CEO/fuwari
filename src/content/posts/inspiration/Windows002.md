...
description: '从 Windows 95 时代开始，「运行」对话框就是 Windows 系统中一个非常实用的功能。通过它，我们可以快速启动系统工具、打开文件夹、运行命令，也能查看或清理常用历史记录。'
image: '/images/windows/ClearHistory.png'
tags: [Windows]
category: '生活灵感'
draft: false 
lang: "zh_CN"
---
从 Windows 95 时代开始，「运行」对话框就是 Windows 系统中一个非常实用的功能。通过它，我们能快速启动各种 Windows 工具、打开文件和文件夹、启动应用，甚至访问网络资源。更方便的是，直接输入常见命令或配合注册表设置，也能一步执行到位。
...
1. 使用 `Win + R` 打开「运行」命令框，输入 `control folders` 打开「文件夹选项」。
```bash
control folders
```

2. 在「常规」选项卡的「隐私」区域中点击「清除」按钮。
